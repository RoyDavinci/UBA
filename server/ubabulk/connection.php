...
function runThread($threadId)
{
	...
	$text1 = mysqli_real_escape_string($newCon, $text1);

	$msisdn1 = preg_replace('/\D/', '', $msisdn1); // Remove non-digit characters

	if (strpos($msisdn1, '234') === 0) {
    		// Already in correct format
	} elseif (strlen($msisdn1) === 10) {
    		// 10-digit local number, add 234
    		$msisdn1 = '234' . $msisdn1;
	} elseif (strlen($msisdn1) === 11 && $msisdn1[0] === '0') {
    		// 11-digit starting with 0, remove 0 and add 234
    		$msisdn1 = '234' . substr($msisdn1, 1);
	}
	...
	$text = $text1;
        ...
        if ($status == 0 && $num == 0) {
            ...
            if (mysqli_affected_rows($newCon) == 1) {
                ...
                $new1 = newCon();
                $res = "Duplicate on insert";

                $recharge = "INSERT INTO temp_messages(msisdn, pages, text, response, created_at, network, request_id)
                                     VALUES ('$msisdn1', '$pages', '$text', '$res', NOW(), '$network', '$requestId')";
                if (!mysqli_query($new1, $recharge)) {
                    $error = mysqli_error($new1);
                    if (strpos($error, 'Duplicate') !== false) {
                        log_action("Duplicate request_id: $requestId already exists in temp_messages.");
                        log_action("[$requestId] Duplicate Request ID");
                        $sql = "INSERT INTO duplicate ( firstname, lastname, batch_id,msisdn, text, sequence) VALUES('$first_name', '$last_name', '$batch', '$msisdn1', '$text', '$sequence')";

                        log_action("[SQL INSERT] " . $sql);
                        if (mysqli_query($new1, $sql)) {
                            log_action("DB SUCCESSFUL [Insert into Duplicate]: ");
                            $del = "DELETE FROM queues WHERE id='$id'";
                            if (!mysqli_query($new1, $del)) {
                                log_action("DB  [Delete from queues after duplicate insert]: ");
                            }
                            mysqli_close($new1);
                        } else {
                            log_action("DB ERROR [Insert into Duplicate/Delete from queues]: " . mysqli_error($new1));
                            mysqli_close($new1);
                        }
                    } else {
                        log_action("DB ERROR [Insert into temp_messages]: " . $error);
                    }
                } else {
                    if (($prefix == "234" || $zero == "0") && $checkforth) {
                        ...
                        if (strpos($response, "Accepted") !== false) {
                            ...
                            $new = newCon();
                            $recharge = "INSERT INTO messages( msisdn, pages, text, response, created_at, network, request_id)
                                     VALUES ('$msisdn1', '$pages', '$text', '$response', NOW(), '$network', '$requestId')";
